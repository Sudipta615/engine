...
/// A fake `Output` at a chosen sample rate, for the multi-endpoint tests.
/// Unlike [`FakeOutput`] (fixed 44.1 kHz), the rate is configurable so a
/// rate-mismatched endpoint exercises the per-endpoint resampler.
pub struct FakeEndpointOutput {
    pub rate: u32,
}

impl FakeEndpointOutput {
    pub fn at_rate(rate: u32) -> Self {
        Self { rate }
    }
}

impl crate::output::OutputVolume for FakeEndpointOutput {
    fn supports_hardware_volume(&self) -> bool {
        false
    }
    fn set_hardware_volume_db(&self, _db: f32) -> Result<(), crate::output::OutputError> {
...
impl crate::output::Output for FakeEndpointOutput {
    ...
    fn output_info(&self) -> crate::output::OutputInfo {
        crate::output::OutputInfo::exclusive("fake-endpoint".to_string(), self.rate, 2, None)
    }
    fn capabilities(&self) -> crate::output::OutputCapabilities {
        crate::output::OutputCapabilities {
            ...
            formats: vec![],
            channels: vec![2],
            device_name: "fake-endpoint".to_string(),
