...
pub enum EngineEvent {
    ...
    /// Room measurement completed: the sweep recording at `path`
    /// was deconvolved and the derived correction IR is now live (enabled,
    /// depth per config).
    MeasurementComplete {
        /// The sweep recording the correction was derived from.
        path: std::path::PathBuf,
        /// Measurement SNR (dB) reported by the sweep deconvolution.
        snr_db: f32,
    },
    /// Room measurement could not complete (no capture backend,
