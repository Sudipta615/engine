...
//! the [`Graph2Engine`] (Phase 46), mirroring
//! `dsp::graph::GraphControlHandle` one-to-one.
//!
...
//! embedded `DspGraph` drains, so command semantics — block-boundary
//! application, sticky user-state mirroring, generation swaps — are the
//! production ones verbatim. The mirror exists so the Phase-47 shadow
//! mode can fan a command out to **both** the active engine and its
//! legacy twin in one call (see `attach_shadow`).

use crate::dsp::correction::CorrectionIrSet;
use crate::dsp::equalizer::EqBandParams;
use crate::dsp::graph::nodes::{DuckState, PanLaw};
use crate::dsp::graph::GraphControlHandle;
...
pub struct Graph2ControlHandle {
    ...
    /// Phase 47 shadow mode: when a legacy twin is attached, commands fan
    /// out to it too, so the A/B bit-compare stays meaningful.
    shadow: Mutex<Vec<GraphControlHandle>>,
}
...
impl Graph2ControlHandle {
    ...
    /// Phase 47: attach a shadow twin (a legacy-plan `DspGraph`'s handle).
    /// Every control call then fans out to it, keeping both graphs
    /// lock-step so the per-block bit-compare is meaningful.
    pub fn attach_shadow(&self, shadow: GraphControlHandle) {
        self.shadow.lock().expect("shadow lock").push(shadow);
    }

    /// Phase 47: detach all shadow twins (shadow mode off).
    pub fn clear_shadow(&self) {
        self.shadow.lock().expect("shadow lock").clear();
    }

    /// Fan a command out to every attached shadow twin (best effort; the
    /// shadow's queues are drained by the comparison driver).
    fn fan_out<F: Fn(&GraphControlHandle)>(&self, f: F) {
        for shadow in self.shadow.lock().expect("shadow lock").iter() {
            f(shadow);
        }
    }
    ...
    pub fn apply_loudness_metadata_outgoing(&self, metadata: Option<LoudnessMetadata>) {
        ...
        self.fan_out(move |h| h.apply_loudness_metadata_outgoing(metadata));
    }
    ...
    pub fn set_crossfeed_custom_params(&self, frequency_hz: f32, q: f32, delay_ms: f32) {
        ...
        self.fan_out(move |h| {
            h.set_crossfeed_custom_params(frequency_hz, q, delay_ms);
        });
    }

    pub fn set_compressor_enabled(&self, enabled: bool) {
        self.inner.set_compressor_enabled(enabled);
        self.fan_out(move |h| h.set_compressor_enabled(enabled));
    }
    ...
    pub fn set_spatial_screen(
        ...
        self.fan_out(move |h| {
            h.set_spatial_screen(center_azimuth_deg, half_width_deg, elevation_deg, gain)
        });
    }
