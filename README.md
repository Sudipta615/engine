...
| **Spatial audio (opt-in)** | An independent spatial scene layer (`spatial/`): world-space objects + listener, named/custom speaker layouts, calibration, an equal-power `BasicPanner`, a 3D **VBAP** renderer (3-triplet solves, 2D reduction, out-of-coverage fallback), and object behavior — **directivity** (cardioid/supercardioid/custom curves), **occlusion** (attenuation + low-pass), and **angular-region spread** — the *same* scene renders to stereo, 5.1, 7.1, 7.1.4, or any custom array into a normal multichannel buffer; conventional PCM/DSP untouched (opt in via `ChannelPolicy::SpatialRender`).
